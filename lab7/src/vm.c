...
static void free_user_pt_level(unsigned long *table, int level) {
    if (table == (void *)0) {
        return;
    }
    ...
    for (int i = 0; i < VM_ENTRIES_PER_TABLE; i++) {
        ...
        if ((entry & (PTE_R | PTE_W | PTE_X)) != 0) {
            if (entry & PTE_U) {
                free_pages(addr_to_page(pte_pa(entry)));
            }
            continue;
        }

        unsigned long *child = (unsigned long *)phys_to_virt(pte_pa(entry));
        if (level > 0) {
            free_user_pt_level(child, level - 1);
        }
        free(child);
    }
}
...
void vm_free_user_pgd(unsigned long *pgd) {
    if (pgd == (void *)0 || pgd == vm_kernel_pgd()) {
        return;
    }
